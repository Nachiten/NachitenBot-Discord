...
import { dateToString, fromUserInputToString } from "../../utils/date-utils";
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option.setName("dia").setDescription("Día a revisar. (Default es hoy)").setRequired(false),
    )
    .addIntegerOption((option) =>
      option.setName("mes").setDescription("Mes a revisar. (Default es actual)").setRequired(false),
    )
    .addIntegerOption((option) =>
      option.setName("año").setDescription("Año a revisar. (Default es actual)").setRequired(false),
    ),
  ...
  async execute(interaction: CommandInteraction) {
    ...
    const selectedDateString = fromUserInputToString(selectedYear, selectedMonth, selectedDay);
    ...
    axios
      ...
      .then(async (response) => {
        ...
        const responseEmbed = new EmbedBuilder()
          ...
          .addFields({
            name: `Personas que no cargaron el ${todayDateString}${isToday}:`,
            value: usersNotSubmitted.join(""),
          })
          .setTimestamp()
